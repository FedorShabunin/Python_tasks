...
class ElectromagneticField():
    ...
    def components_E_field(self, coordinate = np.zeros(4)):
        self.E_components_field = None
        self.E_components_field = np.array([lambdify([x, y, z, t], self.E[i], "numpy")(coordinate[0], coordinate[1], coordinate[2], coordinate[3]) 
                              for i in range(0, 3)], dtype=object)
        return self.E_components_field
            
    def components_H_field(self, coordinate = np.zeros(4)):
        self.H_components_field = None
        self.H_components_field = np.array([lambdify([x, y, z, t], self.H[i], "numpy")(coordinate[0], coordinate[1], coordinate[2], coordinate[3])
                              for i in range(0, 3)], dtype=object) 
        return self.H_components_field
    '''    
    ...
    def module_E(self, coordinate = np.zeros(4)):
        ...
        self.E_module = np.sqrt(np.sum(np.dot(cmp_E, cmp_E)))
        return self.E_module
    
    def module_H(self, coordinate = np.zeros(4)):
        cmp_H = self.components_H_field(coordinate)
        self.H_module = np.sqrt(np.sum(np.dot(cmp_H, cmp_H)))
...
class Particle():
    def __init__(self, mass=0, charge=0, coordinate_initial = np.zeros(3), momentum_initial = np.zeros(3), field=0):
        self.mass = mass
        self.charge = charge
    ...
    def sol_motion_eq(self,):
        ...
        def f(t_1, r):
            ...
            E = self.field.components_E_field(np.append(r[:3], t_1)).astype(np.float64)
            H = self.field.components_H_field(np.append(r[:3], t_1)).astype(np.float64)
            
            f_r = V
            f_P = self.charge*(E + np.cross(V, H))
            f = np.hstack((f_r, f_P))
            return f
...
#Скрещенное:
#E_H_field = ElectromagneticField(E_x='1', E_y=0, E_z=0, H_x=0, H_y='1', H_z=0)
#1) круговая плоская поляризация
E_H_field = ElectromagneticField(E_x=0, E_y='5*cos(2*(t-x))', E_z='5*sin(2*(t-x))', H_x=0, H_y='-5*sin(2*(t-x))', H_z='5*cos(2*(t-x))')
#2) линейная поляризация
#E_H_field = ElectromagneticField(E_x=0, E_y='5*cos(2*(t-x))', E_z=0, H_x=0, H_y=0, H_z='5*cos(2*(t-x))')
#3) стоячая волна
...
electron = Particle(mass=1, charge=1, coordinate_initial=np.array([0, 0, 0]), momentum_initial=np.array([1, 1, 1]), field = E_H_field)
